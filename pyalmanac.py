...
if s in set(['1', '2', '3', '4', '5', '6', '7']):
    if int(s) < 4:
        ...
        input1 = int(input("	Enter MONTH: "))
        input2 = int(input("	Enter Your DAY: "))
        years = input("  ENTER YEAR as yyyy \n")
        if len(years)== 4:
            yearfr = years
            yearto = years
...
""")
    ...
    if s == '1':        # Nautical Almanac (for a year)
        ...
        for yearint in range(int(yearfr),int(yearto)+1):
            ...
            print(msg)
            first_day = datetime.date(yearint, input1, input2)
            fn = "almanac{}{}".format(ff,year+DecFmt)
            outfile = open(fn + ".tex", mode="w", encoding="utf8")
            outfile.write(tables.almanac(first_day, 1)) #122 & deleted "tables.almanac"
            outfile.close()
            stop = time.time()
